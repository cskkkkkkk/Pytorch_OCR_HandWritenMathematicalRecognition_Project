...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv8=nn.Conv2d(64,64,4,stride=1,padding=1)
        self.maxpool5=nn.MaxPool2d(6,stride=1,padding=1)

        self.conv2 = nn.Conv2d(64,128,4,stride=1,padding=1)
        self.maxpool2 = nn.MaxPool2d(3,stride=1,padding=1)
        
        self.conv7=nn.Conv2d(128,128,4,stride=1,padding=1)
        self.maxpool6=nn.MaxPool2d(6,stride=1,padding=1)

        self.conv3 = nn.Conv2d(128,256,4,stride=1,padding=1)
        #self.conv3 = nn.Conv2d(128,256,3,stride=1,padding=1)
        #self.maxpool5= nn.MaxPool2d(2,stride=1,padding=1)


        #self.conv2 = nn.Conv2d(64,256,3,stride=1,padding=1)
        #self.maxpool2 = nn.MaxPool2d(2,stride=1,padding=1)

        self.conv4 = nn.Conv2d(256,256,2,stride=1,padding=1)
        self.maxpool3 = nn.MaxPool2d((2,1),stride=(2,1),padding=(1,0))
        

        self.conv5 = nn.Conv2d(256,512,2,stride=1,padding=1)
    ...
    def forward(self,x):
        ...
        x = self.conv6(x)
        x = F.relu(x)
        #print("encoder6:",x.shape)
        #位置嵌入
        x = x.permute(0,2,3,1)
        #print("encoder7:",x.shape)
        #向特征张量中添加正弦波信号，这些信号的频率和相位随着位置的不同而变化。这样做的目的是让模型能够学习到输入数据中绝对位置和相对位置的信息。
        x = self.add_timing_signal_nd(x)
        #print("encoder8:",x.shape)
        x = x.permute(0,3,1,2)
        #print("encoder9:",x.shape)
        #调用contiguous是为了确保张量在内存中是连续存储的
        x = x.contiguous()
        #print("encoder10:",x.shape)
        return x
#修改自:
...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        #print("attention1:",att1.shape)
        att2 = self.decoder_att(decoder_hidden)  # (batch_size, attention_dim)
        #print("attention2:",att2.shape)
        att = self.full_att(self.relu(att1 + att2.unsqueeze(1))).squeeze(2)  # (batch_size, num_pixels)
        #print("attention3:",att.shape)
        alpha = self.softmax(att)  # (batch_size, num_pixels)
        #print("attention4:",alpha.shape)
        attention_weighted_encoding = (encoder_out * alpha.unsqueeze(2)).sum(dim=1)  # (batch_size, encoder_dim)
        #print("attention5:",attention_weighted_encoding.shape)
    ...
    def forward(self, encoder_out, encoded_captions=torch.zeros(1, 16, device="cuda", dtype=torch.long), caption_lengths=torch.tensor([16], device="cuda"),p = 1):
        ...
        for t in range(int(max(decode_lengths))):
            ...
            attention_weighted_encoding, alpha = self.attention(encoder_out[:batch_size_t],
                                                                h[:batch_size_t])
            #print("decoderwithattention7:",attention_weighted_encoding.shape,alpha.shape)
            gate = self.sigmoid(self.f_beta(h[:batch_size_t]))  # gating scalar, (batch_size_t, encoder_dim)
            #print("decoderwithattention8:",gate.shape)
            attention_weighted_encoding = gate * attention_weighted_encoding
            #print("decoderwithattention9:",attention_weighted_encoding.shape)
            ...
            if t==1 or (np.random.rand() < self.p) :
                ...
                #print("decoderwithattention10:",h.shape)
            else:
                h = self.decode_step(
                    torch.cat([self.embedding(torch.argmax(predictions[:batch_size_t, t, :],dim = 1)), attention_weighted_encoding], dim=1),
                    h[:batch_size_t])  # (batch_size_t, decoder_dim)
                #print("decoderwithattention11:",h.shape)
            preds = self.fc(self.dropout(h))  # (batch_size_t, vocab_size)
            #print("decoderwithattention12:",preds.shape)
            predictions[:batch_size_t, t, :] = preds
            #print("decoderwithattention13:",predictions.shape)
            alphas[:batch_size_t, t, :] = alpha
            #print("decoderwithattention14:",alphas.shape)

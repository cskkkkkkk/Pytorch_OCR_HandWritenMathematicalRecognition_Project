...
from model.dataloader import MyDatasetTest
from config import *
...
def caption_image_beam_search(image,encoder, decoder,  word_map, beam_size=5):
    """
...
if __name__ == '__main__':
    ...
    words=[]
    dataset=MyDatasetTest(testset_dir)
    #test_loader=DataLoader(dataset,batch_size=1,shuffle=collate_fn_MyDataset,num_workers=0)
    for i in range(dataset.__len__()):
        item=dataset.__getitem__(i)
        print(item)
        seq, alphas = caption_image_beam_search(item,encoder, decoder,  word_map)
        #print(seq)
        result = ""  # 初始化一个空字符串
        for i in seq:
            word = reverse_word_map.get(i)  # 获取对应的词
            if word not in ["<start>", "<end>"]:  # 过滤 <start> 和 <end>
                result+=word.strip()
        #word=[reverse_word_map.get(i) for i in seq]
        print(result)
        words.append(result)

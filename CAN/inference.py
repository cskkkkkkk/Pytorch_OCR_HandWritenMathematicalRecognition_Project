...
#     lines = f.readlines()
set=MyDatasetTest(params['test_dir_path'])
...
with torch.no_grad():
    for i in range(size):
        img=set.__getitem__(i)
        img=img.unsqueeze(0).unsqueeze(0)
        img = img.to(device)
        probs, _= model(img, '')
        prediction = words.decode_test(probs)
        predictions.append(prediction)

#保存
    #写入到文件中
with open(params['result_dir'], 'w', encoding='utf-8') as file:
...
#         probs, _= model(img, input_labels, os.path.join(params['decoder']['net'], name))
#         #mae_sum += mae
#         #mse_sum += mse

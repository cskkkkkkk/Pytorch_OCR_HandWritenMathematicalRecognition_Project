...
#     def forward(self, x):
#         # 包装原始模型的 forward，提供额外的默认参数
#         images_mask = torch.ones_like(x)
#         labels = torch.zeros((x.size(0), 10), device=x.device)  # 假设最多10个标签
#         labels_mask = torch.ones((x.size(0), 10), device=x.device)
#         return self.model(x, images_mask, labels, labels_mask)
...
# test_params_flop(wrapped_model, (1, 64, 64))